...
const COUNTRY: Record<string, string> = { SE: t('Sverige'), NO: t('Norge'), DK: t('Danmark'), FI: t('Finland') };

const MEDIA_TYPES: { value: PortfolioMediaType; label: string }[] = [
  { value: 'TikTok', label: t('TikTok-video') },
  { value: 'Instagram', label: t('Instagram-inlägg') },
  { value: 'Video', label: t('Video (länk)') },
  { value: 'Image', label: t('Bild') },
  { value: 'Link', label: t('Annan länk') },
];
...
export function CreatorPortfolioPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!form.title.trim()) { setError(t('Titel krävs')); return; }
    try { new URL(form.mediaUrl); } catch { setError(t('Media-URL måste vara en giltig länk (https://…)')); return; }
    ...
    try {
      ...
      setError(err?.response?.data?.error?.message ?? t('Kunde inte spara'));
    }
  };
  ...
  const handleDelete = (id: string) => {
    ...
    setArmedDelete(null);
    remove.mutate(id, {
      onSuccess: () => toast.push(t('Borttaget ur portföljen'), 'success'),
      onError: () => toast.push(t('Kunde inte ta bort'), 'error'),
    });
  };
        ...
        <div style={{ display: 'flex', gap: 22, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 'min(100%, 240px)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 12, fontSize: 13 }}>
              ...
              {profile?.website && <a href={profile.website} target="_blank" rel="noopener noreferrer" style={{ color: '#C26A4A', fontWeight: 600 }}>{t('Webbplats')}</a>}
            </div>
            {(profile?.profileTags?.length ?? 0) > 0 && <div className="tags" style={{ marginTop: 12 }}>{profile!.profileTags.map((tag) => <span key={tag} className="tag g">{tag}</span>)}</div>}
          </div>
          <Link to="/creator/profile" className="btn-outline" style={{ flex: '0 0 auto' }}>{t('Redigera profil')}</Link>
        </div>
      </div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 16, maxWidth: 860 }}>
          ...
          <form onSubmit={handleSubmit} className="form-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))' }}>
            ...
            <div className="field full"><label>{t('Media-URL')} *</label><input type="url" value={form.mediaUrl} onChange={(e) => setForm({ ...form, mediaUrl: e.target.value })} required placeholder="https://www.tiktok.com/@.../video/…" /></div>
            {(form.mediaType === 'Image' || form.mediaType === 'Video' || form.mediaType === 'Link') && (
              <div className="field full"><label>{t('Miniatyrbild-URL (valfritt)')}</label><input type="url" value={form.thumbnailUrl} onChange={(e) => setForm({ ...form, thumbnailUrl: e.target.value })} placeholder="https://…" /></div>
            )}
            ...
            <div className="pf-card" key={it.id}>
              {(it.mediaType === 'TikTok') ? (
                ...
                <a href={it.mediaUrl} target="_blank" rel="noopener noreferrer" className="pf-img" style={{ display: 'block', backgroundImage: `url(${it.thumbnailUrl || it.mediaUrl})` }}>
                  {it.isFeatured && <div className="pf-flag">{t('Utvald')}</div>}
                </a>
              ) : (
