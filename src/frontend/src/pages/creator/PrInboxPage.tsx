...
const OFFER_TYPE_LABELS: Record<string, string> = {
  ProductGifting: t('Produkt / gåva'),
  Paid: t('Betald'),
  Hybrid: t('Produkt + betalt'),
  Event: 'Event',
};
...
function OfferCard({ offer }: { offer: PrOffer }) {
  ...
  const handleRespond = async (accept: boolean) => {
    ...
    try {
      ...
      setError(err?.response?.data?.error?.message ?? t('Kunde inte svara'));
    }
  };
      ...
      {!open ? (
        ...
        <>
          ...
          <div className="meta-cols" style={{ marginBottom: 14 }}>
            ...
            {offer.productDescription && <div className="mc"><div className="k">{t('Du får')}</div><div className="v">{offer.productDescription}{offer.productValue ? ` (${formatCurrency(offer.productValue)})` : ''}</div></div>}
          </div>
          ...
          {canRespond ? (
            ...
            <p style={{ fontSize: 13, color: 'var(--muted)', marginTop: 10 }}>
              {offer.status === 'Accepted' && t('Du tackade ja.')}
              {offer.status === 'Declined' && t('Du tackade nej.')}
              {offer.status === 'Completed' && t('Slutfört.')}
...
export function CreatorPrInboxPage() {
  ...
  const tabs: { key: typeof tab; label: string; n?: number }[] = [
    { key: 'all', label: t('Alla'), n: offers.length }, { key: 'new', label: t('Nya'), n: newCount }, { key: 'active', label: t('Aktiva'), n: accepted.length }, { key: 'history', label: t('Historik'), n: history.length },
  ];
    ...
    <section className="view active reveal" data-view="proffers">
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">{t('Alla PR-erbjudanden du fått från företag, samlade. Acceptera, följ upp, och se det samlade värdet du behöver redovisa.')}</p>
        </div>
      </div>
      ...
      <div className="vstat-row">
        ...
        <div className="card vstat">
          ...
          <div className="vstat-lbl">{t('Nya erbjudanden')}</div>
          <div className="vstat-val">{newCount}</div>
          <div className="vstat-sub"><span className="vmut">{t('väntar på svar')}</span></div>
        </div>
        <div className="card vstat">
      ...
      {isLoading ? <div className="grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 16 }}><CardSkeleton rows={3} /><CardSkeleton rows={3} /><CardSkeleton rows={3} /></div> : shown.length > 0 ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '54px 24px' }}>
          <div style={{ fontSize: 18, fontWeight: 700 }}>{tab === 'all' ? t('Inga PR-erbjudanden ännu') : t('Inget här ännu')}</div>
          <div style={{ color: 'var(--muted)', fontSize: 14, marginTop: 8, maxWidth: 440, marginInline: 'auto' }}>{t('När företag skickar dig ett PR-erbjudande dyker det upp här. Se till att din profil och portfölj är uppdaterad så fler hittar dig.')}</div>
        </div>
      )}
    </section>
  );
