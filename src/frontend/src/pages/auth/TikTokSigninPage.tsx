...
export function TikTokSigninPage() {
  ...
  useEffect(() => {
    ...
    if (!code || !state) { setError('Inloggningen avbröts.'); return; }
    (async () => {
      try {
        ...
        setError(err?.response?.data?.error?.message ?? 'Kunde inte logga in med TikTok. Försök igen.');
      }
    })();
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FFF4EC', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 420 }}>
        {error ? (
          <>
            <h2 style={{ fontSize: 22, fontWeight: 700, color: '#0B0F17' }}>Hoppsan</h2>
            <p style={{ color: '#6E7480', marginTop: 10, lineHeight: 1.5 }}>{error}</p>
            <a href="/login" style={{ display: 'inline-block', marginTop: 18, background: '#0B0F17', color: '#FFF4EC', padding: '12px 26px', borderRadius: 980, textDecoration: 'none', fontWeight: 600 }}>Till inloggningen</a>
          </>
        ) : (
          <>
            ...
            <p style={{ color: '#6E7480' }}>Loggar in med TikTok…</p>
          </>
        )}
      </div>
    </div>
