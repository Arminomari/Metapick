  ...
  return (
    <section className="view active reveal" data-view="proffers">
      <div className="page-head">
        <div>
          <h1 className="page-title">{t('Dina')} <em>{t('PR-utskick')}</em></h1>
          <p className="page-sub">{t('Skicka direkta PR-erbjudanden till creators och följ varje utskick — sett, accepterat, nekat.')}</p>
        </div>
        <button className="btn-apply" style={{ width: 'auto', padding: '12px 22px', display: 'inline-flex', alignItems: 'center', gap: 8 }} onClick={() => navigate('/brand/creators')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="8" r="3" /><circle cx="16" cy="9" r="2.5" /><path d="M3 19a6 6 0 0 1 12 0M14 18a5 5 0 0 1 7-1" /></svg> {t('Hitta creators')}
        </button>
      </div>
      ...
      {isLoading ? <div className="grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}><CardSkeleton rows={3} /><CardSkeleton rows={3} /><CardSkeleton rows={3} /></div> : data && data.data.length > 0 ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '54px 24px' }}>
          ...
          <div style={{ color: 'var(--muted)', fontSize: 14, marginTop: 8 }}>{t('Hitta creators och skicka ditt första PR-erbjudande.')}</div>
          <button className="btn-apply" style={{ width: 'auto', display: 'inline-block', padding: '11px 22px', marginTop: 16 }} onClick={() => navigate('/brand/creators')}>{t('Hitta creators')}</button>
        </div>
      )}
    </section>
  );
    ...
    <div className="list-row" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 0, minWidth: 0 }}>
      ...
      {open && (
        <div style={{ paddingTop: 12, borderTop: '1px solid rgba(241,168,143,.12)', fontSize: 13.5, minWidth: 0 }}>
          ...
          {offer.responseMessage && <p style={{ marginTop: 8 }}>{t('Creatorns svar:')} <em>"{offer.responseMessage}"</em></p>}
