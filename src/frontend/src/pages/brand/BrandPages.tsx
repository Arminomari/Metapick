...
function getPayoutStatusLabel(status: string) {
  if (status === 'ReadyForManualPayment') return t('Redo att betalas');
  if (status === 'AwaitingThreshold') return t('Väntar på views');
  if (status === 'Completed') return t('Pengar skickade');
  if (status === 'Approved') return t('Godkänd för utbetalning');
  if (status === 'Pending') return t('Väntar');
  if (status === 'Processing') return t('Bearbetas');
  return status;
}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">{t('Direktchatt')}</h3>
        {unreadFromCreator > 0 && (
    ...
    <div className="space-y-12">
      ...
      <header className="grid grid-cols-12 gap-x-6 items-end">
        <div className="col-span-12 md:col-span-8">
          <p className="eyebrow">{t('Brand desk · Översikt')}</p>
          <h1 className="mt-3 text-display text-[clamp(2.25rem,5vw,3.75rem)]">
            {t('Dina')} <span className="text-sunset">{t('kampanjer')}</span>,<br />
            {t('live just nu.')}
          </h1>
        </div>
        <div className="col-span-12 md:col-span-4 md:text-right">
          <p className="text-sm text-muted-foreground col-prose md:ml-auto">
            {t('Lägesbild över aktiva briefs, budget i rörelse, och vad som väntar härnäst.')}
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 md:col-span-4">
          <StatCard label={t('Aktiva kampanjer')}        value={active.length} />
        </div>
        <div className="col-span-6  md:col-span-2">
          <StatCard label={t('Totalt')}                  value={campaigns.length} />
        </div>
        <div className="col-span-6  md:col-span-3">
          <StatCard label={t('Total budget')}            value={formatCurrency(totalBudget)} />
        </div>
        <div className="col-span-12 md:col-span-3">
          <StatCard label={t('Spenderat')}               value={formatCurrency(totalSpent)} subValue={totalBudget ? `${Math.round((totalSpent / totalBudget) * 100)}% ${t('av budget')}` : undefined} />
        </div>
      </div>

      <Card className="!p-0 overflow-hidden">
        <div className="px-6 pt-6 pb-2 flex items-baseline justify-between">
          <h2 className="text-2xl font-bold tracking-tight">{t('Senaste kampanjer')}</h2>
          <span className="eyebrow">{campaigns.length} {t('totalt')}</span>
        </div>
    ...
    <section className="view active reveal">
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">{t('Alla dina briefs på ett ställe. Spåra budget, creators och status i realtid.')}</p>
        </div>
        <button className="btn-apply" style={{ width: 'auto', padding: '12px 22px', display: 'inline-flex', alignItems: 'center', gap: 8 }} onClick={() => navigate('/brand/campaigns/new')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14M5 12h14" /></svg> {t('Ny kampanj')}
        </button>
      </div>

      <div className="tabs">
        {tabs.map((tb) => (
          <button key={tb.label} className={`tab${status === tb.val ? ' active' : ''}`} onClick={() => { setStatus(tb.val); setPage(1); }}>{t(tb.label)}</button>
        ))}
      </div>
        ...
        <div className="card">
          {data?.data.length ? (
            ...
            <div style={{ textAlign: 'center', padding: '44px 24px' }}>
              <div style={{ fontSize: 18, fontWeight: 700 }}>{t('Inga kampanjer än')}</div>
              <div style={{ color: 'var(--muted)', fontSize: 14, marginTop: 8 }}>{t('Skapa din första kampanj för att komma igång.')}</div>
              <button className="btn-apply" style={{ width: 'auto', display: 'inline-block', padding: '11px 22px', marginTop: 16 }} onClick={() => navigate('/brand/campaigns/new')}>{t('Skapa kampanj')}</button>
            </div>
          )}
        </div>
      )}
...
export function CreateCampaignPage() {
  ...
  const payoutSummary = () => {
    ...
    switch (form.payoutModel) {
      case 'Fixed':
        return `${rules[0].amount} ${t('kr per creator som når')} ${formatNumber(rules[0].minViews)} ${t('views')}`;
      case 'CPM':
        return `${rules[0].amount} ${t('kr per 1 000 views')}${rules[0].maxPayoutPerCreator ? ` (max ${rules[0].maxPayoutPerCreator} ${t('kr/creator')})` : ''}`;
      case 'Tiered':
        return rules.map(r =>
          `${formatNumber(r.minViews)}${r.maxViews ? `–${formatNumber(r.maxViews)}` : '+'} ${t('views')} → ${r.amount} kr`
        ).join(' · ');
      default:
        return '';
    }
  ...
  const handleSubmit = async (e: ReactFormEvent) => {
    ...
    if (form.startDate && form.endDate && form.endDate < form.startDate) {
      setError(t('Slutdatumet måste vara efter startdatumet.'));
      return;
    }

    const today = new Date().toISOString().slice(0, 10);
    if (form.endDate && form.endDate <= today) {
      setError(t('Slutdatumet måste vara i framtiden.'));
      return;
    }

    if (form.payoutRules.some(r => r.amount <= 0)) {
      setError(t('Alla utbetalningsbelopp måste vara större än 0.'));
      return;
    }

    const maxCost = estimatedMaxCost();
    if (maxCost > 0 && form.budget < maxCost) {
      setError(`${t('Budget')} (${formatCurrency(form.budget)}) ${t('kan vara för låg. Max kostnad om alla creators får max:')} ${formatCurrency(maxCost)}.`);
    ...
    try {
      ...
      if (resp?.errors) {
        ...
        setError(t('Sessionen har gått ut. Logga ut och in igen.'));
      } else {
        setError(`${t('Fel')} ${err?.response?.status ?? '?'}: ${JSON.stringify(resp ?? err?.message ?? t('okänt fel'))}`);
      }
    }
  };
    ...
    <section className="view active reveal">
      ...
      <div className="card" style={{ maxWidth: 760 }}>
        <form onSubmit={handleSubmit} className="form-grid">
          ...
          <div className="field"><label>{t('Startdatum')} *</label><DateInput value={form.startDate} onChange={v => setForm({ ...form, startDate: v })} required className="" /></div>
          ...
          <div className="field full" style={{ border: '1px solid rgba(241,168,143,.18)', borderRadius: 16, padding: 18, background: 'rgba(255,255,255,.4)', gap: 14 }}>
            ...
            {form.payoutModel === 'Fixed' && (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14 }}>
                  ...
                  <div className="field"><label>{t('Minsta antal views')}</label>
                ...
                <p className="auth-hint">{t('Creatorn får')} {form.payoutRules[0]?.amount ?? 0} {t('kr när videon når')} {formatNumber(form.payoutRules[0]?.minViews ?? 0)} {t('views')}.</p>
              </>
            )}
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14 }}>
                  <div className="field"><label>{t('Pris per 1 000 views (SEK, minst 20)')}</label>
                    <input type="text" inputMode="numeric" value={form.payoutRules[0]?.amount || ''}
                      onChange={(e) => updateRule(0, { amount: Number(e.target.value.replace(/\D/g, '')) || 0 })}
                      placeholder={t('minst 20 kr')} />
                  </div>
                  <div className="field"><label>{t('Max per creator (SEK, valfritt)')}</label>
                ...
                <p className="auth-hint">
                  {(() => {
                    ...
                    return `${t('Exempel: 10 000 views =')} ${formatCurrency(ex10k)}${cap ? ` (max ${formatCurrency(cap)} ${t('per creator')})` : ''}`;
                  })()}
                </p>
              </>
            )}
              ...
              <>
                {form.payoutRules.map((rule, idx) => (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 34px', gap: 10, alignItems: 'end' }}>
                    ...
                    <button type="button" onClick={() => removeTier(idx)} className="lt-icbtn" aria-label={t('Ta bort steg')} title={t('Ta bort steg')} style={{ marginBottom: 4 }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12" /></svg>
                    </button>
                  </div>
                ))}
                <button type="button" onClick={addTier} className="view-all" style={{ alignSelf: 'flex-start' }}>{t('+ Lägg till steg')}</button>
              </>
            )}
            ...
            {payoutSummary() && (
              <div className="vy-alert info" style={{ padding: '12px 15px' }}>
                ...
                <div>
                  <b>{t('Sammanfattning:')}</b> {payoutSummary()}
                  {estimatedMaxCost() > 0 && (
                    <div style={{ marginTop: 3, fontSize: 12, color: 'var(--muted)' }}>{t('Uppskattad maxkostnad')} ({form.maxCreators} {t('creators')}): {formatCurrency(estimatedMaxCost())}</div>
                  )}
                </div>
              </div>
            )}
...
function BrandCampaignTable({ campaigns, onRowClick }: { campaigns: CampaignListItem[]; onRowClick?: (c: CampaignListItem) => void }) {
  const columns: Column<CampaignListItem>[] = [
    { header: t('Namn'), accessor: 'name' },
    { header: t('Status'), accessor: (c) => <StatusBadge status={c.status} /> },
    { header: t('Budget'), accessor: (c) => `${formatCurrency(c.budgetSpent)} / ${formatCurrency(c.budget)}` },
    { header: t('Creators'), accessor: (c) => `${c.approvedCreatorCount} / ${c.maxCreators}` },
    { header: t('Period'), accessor: (c) => `${formatDate(c.startDate)} – ${formatDate(c.endDate)}` },
  ];
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        {cp.videos.map((v: CreatorVideo, i: number) => (
                          <div key={i} className="bg-muted/30 rounded-lg p-3 space-y-2">
                            ...
                            {v.status === 'Approved' && (
                              <span className="inline-block text-xs text-green-400 font-medium">✓ {t('Godkänd')}</span>
                            )}
                            {v.status === 'Rejected' && (
                              <span className="inline-block text-xs text-red-400 font-medium">✗ {t('Nekad')}</span>
                            )}
                          </div>
                        ))}
                      </div>
            ...
            {analytics.creatorPerformance.length === 0 && (
              <div style={{ textAlign: 'center', padding: '34px 24px', color: 'var(--muted)' }}>{t('Inga aktiva creators. Godkänn ansökningar för att se creator-prestanda.')}</div>
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div className="card" style={{ marginBottom: 14 }}>
      ...
      {isLoading ? <CardSkeleton rows={2} /> : applications?.data.length ? (
          ...
          <div key={a.id} className="list-row" style={{ gap: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto' }}>
              ...
              {a.status === 'Pending' && rejectingId !== a.id && (
                <>
                  <button className="btn-apply" style={{ width: 'auto', padding: '9px 16px', fontSize: 12.5 }} onClick={() => approve.mutateAsync({ id: a.id })} disabled={approve.isPending}>{t('Godkänn')}</button>
                  <button className="btn-outline" style={{ padding: '9px 16px', fontSize: 12.5 }} onClick={() => { setRejectingId(a.id); setRejectReason(''); }} disabled={reject.isPending}>{t('Neka')}</button>
                </>
              )}
              {a.status === 'Pending' && rejectingId === a.id && (
        ...
        <div style={{ padding: '24px 6px', textAlign: 'center', color: 'var(--muted)' }}>{t('Inga creators har ansökt till denna kampanj ännu.')}</div>
      )}
    </div>
  );
}
...
export function BrandApplicationsPage() {
  ...
  return (
    <section className="view active reveal">
      <div className="page-head">
        <div>
          <h1 className="page-title">{t('Ansökningar')}</h1>
          <p className="page-sub">{t('Hantera ansökningar från creators till dina kampanjer. Godkänn de som passar din brief, neka resten.')}</p>
        </div>
      </div>
      {relevantCampaigns.length ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '54px 24px' }}>
          <div style={{ fontSize: 18, fontWeight: 700 }}>{t('Inga kampanjer')}</div>
          <div style={{ color: 'var(--muted)', fontSize: 14, marginTop: 8 }}>{t('Skapa en kampanj för att börja ta emot ansökningar.')}</div>
        </div>
      )}
    </section>
  );
...
export function BrandSettingsPage() {
  ...
  const handlePasswordChange = async (e: ReactFormEvent) => {
    ...
    if (pwForm.newPassword !== pwForm.confirm) {
      setPwError(t('Lösenorden matchar inte.'));
      return;
    }
    if (pwForm.newPassword.length < 8) {
      setPwError(t('Lösenordet måste vara minst 8 tecken.'));
      return;
    }
    try {
      await changePassword.mutateAsync({ currentPassword: pwForm.currentPassword, newPassword: pwForm.newPassword });
      setPwMsg(t('Lösenordet ändrades!'));
      setPwForm({ currentPassword: '', newPassword: '', confirm: '' });
    } catch (err: any) {
      const msg = err?.response?.data?.error?.message;
      setPwError(msg ?? t('Kunde inte ändra lösenordet. Kontrollera ditt nuvarande lösenord.'));
    }
  };
      ...
      {activeTab === 'profile' && (
        <div className="card" style={{ maxWidth: 720 }}>
          ...
          <form onSubmit={handleProfileSave} className="form-grid">
            ...
            <div className="field full"><label>{t('Webbplats')}</label><input type="url" value={profileForm.website} placeholder={t('https://mittföretag.se')} onChange={e => setProfileForm({ ...profileForm, website: e.target.value })} /></div>
            <div className="field full"><label>{t('Beskrivning')}</label><textarea value={profileForm.description} rows={3} onChange={e => setProfileForm({ ...profileForm, description: e.target.value })} placeholder={t('Berätta om ert företag...')} /></div>
            <div className="field full">
              {profileError && <p style={{ color: 'var(--red)', fontSize: 13 }}>{profileError}</p>}
              {profileMsg && <p style={{ color: '#2f9d5b', fontSize: 13 }}>{profileMsg}</p>}
              <button type="submit" className="btn-apply" style={{ width: 'auto', padding: '12px 22px', marginTop: 4 }} disabled={updateProfile.isPending}>{updateProfile.isPending ? t('Sparar…') : t('Spara ändringar')}</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="card" style={{ maxWidth: 720 }}>
          ...
          <form onSubmit={handlePasswordChange} className="form-grid">
            ...
            <div className="field full">
              ...
              <button type="submit" className="btn-apply" style={{ width: 'auto', padding: '12px 22px', marginTop: 4 }} disabled={changePassword.isPending}>{changePassword.isPending ? t('Ändrar…') : t('Byt lösenord')}</button>
            </div>
          </form>
        </div>
      )}
