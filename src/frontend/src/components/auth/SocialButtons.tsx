...
export function SocialButtons({ onToken, disabled }: {
  ...
  const tiktokStart = async () => {
    ...
    try {
      ...
      setBusy(null);
      setError(t('Kunde inte starta TikTok-inloggning — försök igen'));
    }
  };
  ...
  const run = async (name: string, fn: () => Promise<SocialTokenResult>) => {
    ...
    try {
      ...
      if (!/avbröts/.test(msg)) setError(msg || t('Inloggningen misslyckades — försök igen'));
    } finally {
      setBusy(null);
    }
  };
  ...
  return (
    <div className="social-block">
      <div className="auth-divider"><span>{t('eller fortsätt med')}</span></div>
      <div className="social-row">
        ...
        >
          ...
          {busy === 'tiktok' ? t('Öppnar…') : t('Fortsätt med TikTok')}
        </button>
        {providers?.google.enabled && <div ref={googleHost} className="gsi-host" aria-label={t('Fortsätt med Google')} />}
        {providers?.apple.enabled && (
          ...
          >
            ...
            {busy === 'apple' ? t('Öppnar…') : t('Fortsätt med Apple')}
          </button>
        )}
        {providers?.facebook.enabled && (
          ...
          >
            ...
            {busy === 'facebook' ? t('Öppnar…') : t('Fortsätt med Facebook')}
          </button>
        )}
      </div>
