...
export function CreatorTapsSection() {
  ...
  if (isLoading || taps.length === 0) return null;

  return (
    <div className="card" style={{ marginTop: 18, background: 'linear-gradient(160deg,#fff,#FFF6F0)' }}>
      <div className="vperf-head">
        <h3>💧 {t('Dina kranar')}</h3>
        <span style={{ fontSize: 13, color: 'var(--muted)' }}>{t('Löpande ersättning — publicera när du vill')}</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14, marginTop: 6 }}>
        {taps.map((tap) => {
          ...
          return (
            <div key={tap.tapId} onClick={() => navigate(`/creator/assignments/${tap.assignmentId}`)} style={{ cursor: 'pointer', border: '1px solid rgba(241,168,143,.25)', borderRadius: 18, padding: 16, background: '#fff', boxShadow: '0 8px 24px rgba(180,120,90,.07)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {tap.brandLogoUrl
                  ? <img src={tap.brandLogoUrl} alt="" style={{ width: 38, height: 38, borderRadius: 11, objectFit: 'cover' }} />
                  : <span style={{ width: 38, height: 38, borderRadius: 11, background: 'linear-gradient(135deg,#FFD8C7,#F1A88F)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800 }}>{(tap.brandName[0] || '?').toUpperCase()}</span>}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 14.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tap.brandName}</div>
                  <div style={{ fontSize: 12, color: 'var(--muted)' }}>{tap.cpm} kr / 1 000 views{tap.monthlyCapPerCreator ? ` · ${t('max')} ${formatCurrency(tap.monthlyCapPerCreator)}/${t('mån')}` : ''}</div>
                </div>
                <span className={`badge ${open ? 'green' : 'grey'}`}>{open ? t('Öppen') : t('Pausad')}</span>
              </div>
              ...
              <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--muted)', display: 'flex', justifyContent: 'space-between' }}>
                <span>{t('Kranen')}: {tapPct}% {t('använd')}</span>
                <span>#{tap.requiredHashtag}</span>
              </div>
    ...
    <div className="card" style={{ marginBottom: 16, background: 'linear-gradient(160deg,#fff,#FFF6F0)', border: '1px solid rgba(241,168,143,.4)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          <div style={{ fontWeight: 800, fontSize: 15 }}>{t('Kranen')} · {tap.brandName} <span className={`badge ${open ? 'green' : 'grey'}`} style={{ marginLeft: 6 }}>{open ? t('Öppen') : t('Pausad')}</span></div>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--muted)' }}>{t('denna månad')} · {formatCurrency(tap.myLifetimeEarned)} {t('totalt')}</div>
        </div>
      </div>
      <div style={{ marginTop: 12, padding: '12px 14px', borderRadius: 13, background: 'rgba(255,255,255,.75)', border: '1px solid rgba(241,168,143,.2)' }}>
        ...
        <p style={{ margin: '6px 0 0', fontSize: 13.5, lineHeight: 1.6, whiteSpace: 'pre-line' }}>{tap.brief}</p>
        {tap.contentInstructions && <p style={{ margin: '8px 0 0', fontSize: 13, lineHeight: 1.6, color: 'var(--ink-2)', whiteSpace: 'pre-line' }}>{tap.contentInstructions}</p>}
